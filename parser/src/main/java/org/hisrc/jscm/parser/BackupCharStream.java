...
/**
 * An implementation of interface CharStream. Modified extensively by Brian
 * Goetz (Oct 2000) to support being able to back up in the buffer. Modified to
 * support Unicode input. Convenience one-arg constructor provided.
 */
...
public final class BackupCharStream implements CharStream {
	...
	private final void FillBuff() throws java.io.IOException {
		...
		try {
			int i = inputStream.read(curBuf.buffer, curBuf.dataLen, curBuf.size
					- curBuf.dataLen);
			if (i == -1) {
				inputStream.close();
...
//				// exception handling
//				throw new ParseException("MalformedInput");
//			}
			throw e;
		}
	}
	...
	public final void backup(int amount) {
		...
		if (curBuf.curPos - amount < 0) {
			int addlChars = amount - (inputStreamClosed ? 0 : 1)
					- curBuf.curPos;
			curBuf.curPos = 0;
			swapBuf();
	...
	public final String GetImage() {
		...
		if (tokenBeginBuf == curBuf) {
			ret = new String(curBuf.buffer, tokenBeginPos, curBuf.curPos
					- tokenBeginPos + 1);
		} else {
			ret = new String(otherBuf.buffer, tokenBeginPos, otherBuf.dataLen
					- tokenBeginPos);
			if (curBuf.curPos < curBuf.dataLen)
	...
	public final char[] GetSuffix(int len) {
		...
		if ((curBuf.curPos + 1) >= len)
			System.arraycopy(curBuf.buffer, curBuf.curPos - len + 1, ret, 0,
					len);
		else {
			if (otherBuf.dataLen >= len - curBuf.curPos - 1)
				System.arraycopy(otherBuf.buffer, otherBuf.dataLen
						- (len - curBuf.curPos - 1), ret, 0, len
						- curBuf.curPos - 1);
